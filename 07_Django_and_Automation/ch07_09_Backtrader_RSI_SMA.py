...
class MyStrategy(bt.Strategy):
    ...
    def notify_order(self, order):  #1
        if order.status in [order.Submitted, order.Accepted]:
            return
        if order.status in [order.Completed]:   #2
            if order.isbuy():
...
cerebro.broker.setcommission(commission=0.0014) #4
cerebro.addsizer(bt.sizers.PercentSizer, percents=90) #5

print(f'Initial Portfolio Value : {cerebro.broker.getvalue():,.0f} KRW')
cerebro.run()
print(f'Final Portfolio Value : {cerebro.broker.getvalue():,.0f} KRW')
cerebro.plot(style='candlestick') #6
